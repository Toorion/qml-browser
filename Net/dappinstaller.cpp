    ...
    QFuture<void> future = QtConcurrent::run([&, url, targetDir, tag]{
        ...
        if(isRepository(targetDir)) {
            if(tag.empty()) {
                ...
                mergeToHead(targetDir.toLocal8Bit().data());
                qDebug() << "OK" << ok << git_error_last()->message;
            } else {
                QList<QString> tags = localLs(targetDir.toLocal8Bit().data());
                std::string localTag = getLatestBranch(tags);
        ...
        if(ok && tag.length() > 0) {
            emit progressOutput(QString::fromUtf8("Swithing to tag " + tag));
...
bool DappInstaller::isRepository(const QString &targetDir)
{
    ...
    if (err < 0) {
        qDebug() << "OpenRepo err" << git_error_last()->message;
    }
...
bool DappInstaller::fetch(const QString &targetDir, const QString remoteName)
{
    ...
    if (stats->local_objects > 0) {
        printf("\rReceived %u/%u objects in %u bytes (used %u local objects)\n",
               stats->indexed_objects, stats->total_objects, stats->received_bytes, stats->local_objects);
    } else{
        printf("\rReceived %u/%u objects in %u bytes\n",
               stats->indexed_objects, stats->total_objects, stats->received_bytes);
    }

    qDebug() << "FETCH:"<< stats->indexed_objects << stats->total_objects << stats->received_bytes;
...
on_error:
    qDebug() << "FETCH ERROR" << git_error_last()->message;
    git_remote_free(remote);
    return -1;
}
...
bool DappInstaller::clone(const QUrl &url, const char *path)
{
    if (git_libgit2_init() < 0) {
        qWarning("Git init error!");
        emit error("Git init error");
        return false;
    }
    ...
    if (err != 0) {
        emit progressChanged(0, "Error cloning repository: " + QString(git_error_last()->message));
        emit progressOutput("Error cloning repository: " + QString(git_error_last()->message));
        return false;
    }
...
void DappInstaller::checkout(const char *path, const std::string tag)
{
    git_repository *repo = nullptr;
    ...
    qDebug() << "Checkout ref" << tagRef;

    if ((err = resolve_refish(&annotated_commit, repo, tagRef)) < 0 &&
        (err = guess_refish(&annotated_commit, repo, tagRef)) < 0) {
        fprintf(stderr, "failed to resolve %s: %s\n", tagRef, git_error_last()->message);
        //goto cleanup;
    }
    ...
    if (err < 0) {
        qDebug() << "CheckoutTree err" << git_error_last()->message;
    }
...
bool DappInstaller::mergeToHead(const char *path)
{
    ...
    // err = git_branch_upstream(&tracking_ref, branch_ref);

    // err = git_annotated_commit_from_ref(&annotated_commit, repo, tracking_ref);
    ...
    err = git_merge_analysis(&analysis, &preference,
                             ...
                             1);
    if (err != 0) {
        qDebug() << "MERGE ERR" << git_error_last()->message;
        goto cleanup;
    }
...
int DappInstaller::fetchProgress(const git_indexer_progress *stats, void *payload)
{
    ...
    // qDebug() << "NN" << stats->total_objects << stats->received_objects;
    // qDebug() << "NI" << stats->indexed_objects << stats->received_objects;
...
QList<QString> DappInstaller::remoteLs(char *url)
{
    ...
    if (error < 0) {
        m_error = 1;
        m_errorMessage = git_error_last()->message;
        goto cleanup;
    }
